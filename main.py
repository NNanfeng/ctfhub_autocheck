...
driver.find_element(By.ID,'account').send_keys('NNanfeng')
driver.find_element(By.ID,'password').send_keys('cc650008')
# driver.find_element_by_id('account').clear()
# driver.find_element_by_id('password').clear()
# driver.find_element_by_id('account').send_keys('NNanfeng')
# driver.find_element_by_id('password').send_keys('cc650008')
sleep(1)
cjy = Chaojiying_Client('NNanfeng', 'cc650008', '938976')
while True:
    sleep(1)#这个sleep是我当时想先看看输入的验证码对不对而写的
    img = driver.find_element(By.CLASS_NAME,'getCaptcha').get_attribute('src')
    # img = driver.find_element_by_class_name('getCaptcha').get_attribute('src')
    #print(img)
    ...
    # driver.find_element_by_id('imgCaptcha').send_keys(dic['pic_str'])#写入得到的验证码
    sleep(1)
    driver.find_element(By.ID,'imgCaptcha').send_keys(Keys.ENTER)  # 写入enter，代替了  点击[确定]键 的行为
    # driver.find_element_by_id('imgCaptcha').send_keys(Keys.ENTER)#写入enter，代替了  点击[确定]键 的行为
    sleep(3)
...
# sleep(3)
message_url = 'https://sctapi.ftqq.com/SCT170521TV1trUAxQDfwuKUWJSVDtNTD3.send'
post = {
    'title' : "CTFhub自动签到",
    'desp' : "签到成功",
    'short' : "签到成功"
